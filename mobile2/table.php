...
<html>
    ...
    <header >
      ...
      <div class="col-6 pt-1">
        <span class="navbar-brand" onclick="openNav()"><i class="fa fa-bars" aria-hidden="true"></i> Order</span>
      </div>
        <div class="col-6 pt-1">
      <span id="fiyat"></span>
        </div>
  ...
  <nav>
            ...
            <div class="row">
            <div class="tab-content col-3 tab-khanos-content" id="nav-tabContent">
                <?php
                $say = 0;
                $result = mysqli_query($conn, 'SELECT * FROM menu_types order by Ord ASC;');
                while (($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) != NULL) {
                    ?>
                    <div class="tab-pane fade tab-pane-khanos  <?php if (isset($_GET['menu']) && $_GET['menu'] == $row['id']) { ?>active <?php } else if (!isset($_GET['menu']) && $say == 0) { ?>active<?php } ?>" id="<?php
                    echo $row['id'];
                    $menu_id = $row['id'];
                    ?>">                      
                             <?php
...
<button id='button' value="modules/table/modal.php?item=<?php echo $row['id']; ?>&id=<?php echo $_GET['id']; ?>" class="<?php echo $row['classes']; ?>" onclick='f1(this)'><?php echo str_replace("==", "", $row['Menu_Item_Name']) ;  ?></button> 
</div>
                                <?php } else { ?>
                                    ...
                                    <div class="foo kh-btn-box <?php echo $row['classes']; ?>">
                                        <form method="POST"  action="modules/table/operations.php" id="test-form" enctype="multipart/form-data">
                                            ...
                                            <button type="submit" class="<?php echo $row['classes']; ?>" ><?php echo str_replace("==", "", $row['Menu_Item_Name']) ;  ?></button>
                                        </form>    
                                    </div>
...
<a class="nav-item flex-sm-fill text-sm-center btn btn-lg btn-outline-danger " href="order_cancel.php?table=<?php echo $_GET['id'] ?>"> <i class="fa fa-trash-o" aria-hidden="true"></i> Cancel</a>

<a class="nav-item flex-sm-fill btn-lg text-sm-center btn btn-outline-success " href="print_order.php?func=done&table=<?php echo $_GET['id'] ?>"> <i class="fa fa-print" aria-hidden="true"></i> Done</a>
